...
/*
 * FILE          include/nocheck/nocheck.h
 *
 * AUTHORS
...
//
// @breaf Allocates memory for specified type. Essensialy shorthand for NOC_MALLOC(sizeof(TYPE))
...
typedef enum {
    NOC_TEST_CASE_STATUS_OK,
    NOC_TEST_CASE_STATUS_FAILED
} NOC_TestCaseStatus;
...
#define NOC_TASSERT(CASE, EXPRESSION) \
    do { \
        if (!(EXPRESSION)) { \
            ...
            NOC_TestCaseAddCheckFailure((CASE), NOC_TEST_CHECK_TYPE_ASSERT, NOC_STRINGIFY(EXPRESSION), __FILE__, __LINE__); \
            return; \
        } \
    } while (0)
...
#define NOC_TEXPECT(CASE, EXPRESSION) \
    do { \
        if (!(EXPRESSION)) { \
            (CASE)->status = NOC_TEST_CASE_STATUS_FAILED; \
            NOC_TestCaseAddCheckFailure((CASE), NOC_TEST_CHECK_TYPE_EXPECT, NOC_STRINGIFY(EXPRESSION), __FILE__, __LINE__); \
        } else { \
